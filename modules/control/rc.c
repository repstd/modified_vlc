...
#define _RC_DEBUG_FILE_NAME  "D://TEST//rclog.txt"
#ifndef MAX_LINE_LENGTH
#define MAX_LINE_LENGTH 1024
#endif

char rc_debug_buf[MAX_LINE_LENGTH];
FILE* rc_debug_fout;
#define _RC_DEBUG_LOG_INIT(filename) \
        rc_debug_fout=fopen(filename,"w");\
        fclose(rc_debug_fout);

#define _RC_DEBUG_ENCODE_MESSAGE(filename,format,data,...)\
            rc_debug_fout=fopen(filename,"a");\
...
#ifdef _RC_DEBUG 
    #define RC_DEBUG_LOG_INIT()  _RC_DEBUG_LOG_INIT(_RC_DEBUG_FILE_NAME)
    #define RC_DEBUG_ENCODE_MESSAGE(format,data,...) _RC_DEBUG_ENCODE_MESSAGE(_RC_DEBUG_FILE_NAME,format,data,...)
#else 
    #define RC_DEBUG_LOG_INIT()
    #define RC_DEBUG_ENCODE_MESSAGE(format,data,...)
#endif
...
#define rc_debug_vlc_module_begin() \
...
{ \
    ...
    vlc_plugin_set (VLC_MODULE_CREATE, &module);\
    //RC_DEBUG_ENCODE_MESSAGE("filename: %s\n",module.psz_filename);\
    vlc_module_set (VLC_MODULE_NAME, (MODULE_STRING));

#define rc_debug_vlc_module_begin_naked() \
EXTERN_SYMBOL DLL_SYMBOL \
int CDECL_SYMBOL __VLC_SYMBOL(vlc_entry) (vlc_set_cb, void *); \
EXTERN_SYMBOL DLL_SYMBOL \
int CDECL_SYMBOL __VLC_SYMBOL(vlc_entry) (vlc_set_cb vlc_set, void *opaque) \
...
#define rc_debug_set_callbacks( activate, deactivate ) \
    if(vlc_module_set (VLC_MODULE_CB_OPEN, activate))\
        RC_DEBUG_ENCODE_MESSAGE("%s\n","Failed to active;")\
    else\
        RC_DEBUG_ENCODE_MESSAGE("%s\n","Success to Active;")\
...
#define rc_debug_set_callbacks_naked( activate, deactivate ) vlc_module_set (VLC_MODULE_CB_OPEN, activate)
...
vlc_module_begin()
    ...
    //rc_debug_set_callbacks_naked( Activate, Deactivate )
 ...
 *****************************************************************************/
 ...
 void rc_debug_vlc_UrlParse (vlc_url_t * url, const char *str, unsigned char opt)          // sunqueen modify
{
    ...
    //RC_DEBUG_ENCODE_MESSAGE("host:port %s\n",cur);
...
static int Activate( vlc_object_t *p_this )
{
        ...
        ( psz_host = var_InheritString( p_intf, "rc-host" ) ) != NULL )
    {
        ...
        vlc_url_t url;
        //yulw modified: rc_debug
        //vlc_UrlParse( &url, psz_host, 0 );
        //rc_debug_vlc_UrlParse(&url,psz_host,0);
        vlc_UrlParse(&url,psz_host,0);
        ...
        RC_DEBUG_ENCODE_MESSAGE("%s\n","Using RC-Host Mode.");
        RC_DEBUG_ENCODE_MESSAGE("Protocol: %s\n",url.psz_protocol);

        if(url.psz_username!=NULL)
            RC_DEBUG_ENCODE_MESSAGE("Name: %s\n",url.psz_username);
        if(url.psz_password!=NULL)
            RC_DEBUG_ENCODE_MESSAGE("Password: %s\n",url.psz_password);
        RC_DEBUG_ENCODE_MESSAGE("Host: %s\n",url.psz_host);
        RC_DEBUG_ENCODE_MESSAGE("Port: %d\n",url.i_port);

#endif 

        //yulw modified 
        //Here is the buggy point which leads to the dead socket when using remote-control. 
/*        msg_rc("base: %s, port: %d", url.psz_host, url.i_port);
        ...
        if( pi_socket == NULL )
        {
            ...
            RC_DEBUG_ENCODE_MESSAGE("can't listen to %s port %d",url.psz_host, url.i_port );
    ...
    else
    {
        ...
        #ifdef _RC_DEBUG
        RC_DEBUG_ENCODE_MESSAGE("%s","not using --rc-host mode.\n");
        #endif
    }
    ...
    while( vlc_object_alive( p_intf ) )
    {
            ...
            p_intf->p_sys->i_socket == -1 )
        {
            ...
            else
            {
                ...
                RC_DEBUG_ENCODE_MESSAGE("connected to socket: %d\n",p_intf->p_sys->i_socket);
            }
