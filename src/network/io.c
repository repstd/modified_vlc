...
#define _RC_DEBUG_FILE_NAME  "D://TEST//rclog_io.txt"
...
#else 
    //nothing to be done if _RC_DEBUG not defined
                 ...
                 int i_port, int type, int protocol)
{
    ...
    if (i_val)
    {
        ...
        RC_DEBUG_ENCODE_MESSAGE("Cannot resolve %s port %d : %s",
                 (psz_host != NULL) ? psz_host : "", i_port,
                 gai_strerror (i_val));
    ...
    for (struct addrinfo *ptr = res; ptr != NULL; ptr = ptr->ai_next)
    {
        ...
        RC_DEBUG_ENCODE_MESSAGE("Trying to bind socket%d \n",fd)
        if (fd == -1)
        {
