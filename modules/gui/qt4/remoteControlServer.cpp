...
char* RCUtil::addItem(int item,char* dst) {
    char* temp;
    if(asprintf(&temp,"%d",item)==-1)
        return dst;
    char* result=RCUtil::addItem(temp,dst);
    free(temp);
    return result;
}
...
const char* RCUtil::getTotalTrackList(const vlc_value_t& val,const vlc_value_t& text,char* result,int type)
{
    ...
    char temp[1000];
    int len;
    vector<pair<string,string> > track;
    for(int i=0;i<val.p_list->i_count;i++) {
        if(!text.p_list->p_values[i].psz_string||!strlen(text.p_list->p_values[i].psz_string))
            continue;
        memset(temp,0,sizeof(temp));
        sprintf(temp,"%d",val.p_list->p_values[i].i_int);
        track.push_back(std::make_pair(temp,text.p_list->p_values[i].psz_string));
    }
    //sort(track.begin(),track.end(),TrackListCmp());
    for(vector<pair<string,string> >::iterator iter=track.begin();iter!=track.end();iter++) {
        switch(type) {
            case RCUtil::STRING:
                p=RCUtil::addItem(const_cast<char*>(iter->second.c_str()),p);
                break;
            case RCUtil::INT:
                p=RCUtil::addItem(const_cast<char*>(iter->first.c_str()),p);
                break;
            case RCUtil::FULL:
            default:
                p=RCUtil::addItem(const_cast<char*>((iter->first+"`"+iter->second).c_str()),p);
                break;
        }
    }
    if(p==result)
...
int BasicCommand::BasicControl(intf_thread_t* p_intf, const char*psz_cmd, rc_value_t& newval, char* p_data) 
{
            ...
            !strcmp(psz_cmd, "title_p"))
    {
        if (!strcmp(psz_cmd, "getTitle"))
        {
            ...
            !strcmp(psz_cmd,"getAllSTrackFull")||
            !strcmp(psz_cmd,"getAllATrack")||
            !strcmp(psz_cmd,"getAllATrackFull")||
            !strcmp(psz_cmd,"getAllVTrack")||
            !strcmp(psz_cmd,"getAllVTrackFull")
           ) 
    {
        ...
        if(result>=0) {
            ...
            if(strstr(psz_cmd,"Full"))
                totalTrackList=RCUtil::getTotalTrackList(val,text,p_data,RCUtil::FULL);
            else
                totalTrackList=RCUtil::getTotalTrackList(val,text,p_data,RCUtil::INT);
            ENCODE_MSG_WITH_RETURN(p_data,"%s",totalTrackList);
        }
        else
            ENCODE_MSG_WITH_RETURN(p_data, "%s", "{}");
