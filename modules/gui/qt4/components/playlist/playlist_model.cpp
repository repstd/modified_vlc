...
{
    logger::inst()->log(TAG_DEBUG,"PLModel dropMove [target:%s]\n",target->p_input->psz_name);
    QList<input_item_t*> inputItems = plMimeData->inputItems();
    ...
    foreach( input_item_t *p_input, inputItems )
    {
        logger::inst()->log(TAG_DEBUG,"found input_items %s\n",p_input->psz_uri);
        playlist_item_t *p_item = playlist_ItemGetByInput( p_playlist, p_input );
        if( !p_item ) continue;
    ...
    char* temp;
    asprintf(&temp,"[%s] [parent:%s] [count:%d] [new_pos:%d]","PLModel TreeMoveMany",p_parent->p_input->psz_name,i,new_pos);
